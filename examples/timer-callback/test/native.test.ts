...
describe('timer-callback', () => {
  ...
  describe('concurrent cancellable streams', () => {
    ...
    it('cleans live subscriptions during addon teardown', () => {
      const script = `
        const addon = require(${JSON.stringify(path.join(__dirname, '..', 'dist_swift-node', `${process.platform}-${process.arch}`, `timer_callback.${process.platform}-${process.arch}.node`))})
        let events = 0
