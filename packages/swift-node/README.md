...
`swift-node` turns annotated Swift functions into a Node-API addon and generates TypeScript declarations. It keeps its generated runtime and target-qualified native binaries in `dist_swift-node/`. Every annotated function becomes a named JavaScript export; there is no default export.
...
Parses annotated functions, generates the bridge, and builds `dist_swift-node/{platform}-{arch}/{moduleName}.{platform}-{arch}.node`.
...
Checks the required local toolchain.
...
## Ship target binaries (optional)

You only need prebuilt binaries when publishing a package for machines that should not compile Swift locally. During interactive `init`, choose the GitHub runner targets you want and opt into the generated publish workflow.

The generated `CI` workflow builds each target in parallel with the package manager you chose. That one normal `build` script also creates the target-qualified binary. A final Ubuntu job collects the binaries and runtime files, packs one npm tarball, and can publish it after `CI` succeeds on `main`. There is no `swift-node prebuild` command.
...
Publish the target-qualified `dist_swift-node/{platform}-{arch}/{moduleName}.{platform}-{arch}.node` binaries, or `dist/{platform}-{arch}/{moduleName}.{platform}-{arch}.node` when using tsdown. The generated manifest and workflow already include the required runtime sidecars. Start with the [prebuild workflow template](./templates/prebuild.yml) when adding this to an existing package.
