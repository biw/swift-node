 ...
 * they never ship an assembly-machine binary by accident. macOS binaries live
 * at the output root; Linux and Windows retain target directories for Swift
 * runtime sidecars.
 */
      ...
      : [
          ...
          ...targets.map((target) =>
            target.platform === 'darwin'
              ? `dist_swift-node/*.${target.id}.node`
              : `dist_swift-node/${target.id}/*.${target.id}.node`,
          ),
...
export function generatePrebuildWorkflow(
  ...
  const matrix = targets.flatMap((target, index) => [
    ...
    `            prebuild_path: ${
      target.platform === 'darwin'
        ? `${outputDirectory}/${moduleName}.${target.id}.node`
        : `${outputDirectory}/${target.id}/**`
    }`,
  ...
  return [
    ...
    '          for target in $TARGETS; do',
    '            case "$target" in',
    `              darwin-*) test -f "${outputDirectory}/$MODULE_NAME.$target.node" ;;`,
