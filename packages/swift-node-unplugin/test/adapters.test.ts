...
function withProject(callback: (projectDir: string) => Promise<void>): Promise<void> {
  ...
  writeFileSync(path.join(generatedDirectory, nativeFilename), 'native')
  // Linux-style sidecar keeps this adapter test platform independent; real
  // Windows builds use .dll instead.
  writeFileSync(path.join(generatedDirectory, runtimeFilename), 'swift runtime')
...
function expectAssets(outputDirectory: string): void {
  expect(existsSync(path.join(outputDirectory, 'server', nativeFilename))).toBe(true)
  expect(existsSync(path.join(outputDirectory, 'server', runtimeFilename))).toBe(true)
}
...
describe('public Unplugin adapters', () => {
  ...
  it('places sidecars beside a relative esbuild outfile using absWorkingDir', async () => {
    await withProject(async (projectDir) => {
      ...
      expect(existsSync(path.join(outputDirectory, nativeFilename))).toBe(true)
      expect(existsSync(path.join(outputDirectory, runtimeFilename))).toBe(true)
    })
  })
})
