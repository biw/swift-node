...
describe('swiftNodeNativeAssets', () => {
  it('runs the project-local swift-node build before it emits native assets', async () => {
    await withProject(async (projectDir) => {
      ...
      expect(emitted[0]?.fileName).toBe('darwin-arm64/my_addon.darwin-arm64.node')
  ...
  it('emits only exact target-qualified binaries while leaving JavaScript to the bundler', async () => {
    await withProject(async (projectDir) => {
      ...
      mkdirSync(path.join(generatedDirectory, 'darwin-arm64'))
      ...
      writeFileSync(
        path.join(generatedDirectory, 'darwin-arm64', 'my_addon.darwin-arm64.node'),
        'darwin',
      )
      ...
      expect(emitted.map((asset) => asset.fileName)).toEqual([
        'darwin-arm64/my_addon.darwin-arm64.node',
...
const native = require(path.join(__dirname, process.platform + '-' + process.arch, 'my_addon.' + process.platform + '-' + process.arch + '.node'))
...
`,
      ...
      expect(
        existsSync(path.join(projectDir, 'dist', 'darwin-arm64', 'my_addon.darwin-arm64.node')),
      ).toBe(true)
