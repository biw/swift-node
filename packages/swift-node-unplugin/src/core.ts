...
/**
 * Returns only target-qualified binaries generated for this package. macOS
 * binaries live at the generated-output root; Linux and Windows binaries stay
 * in target directories with their Swift runtime sidecars. This intentionally
 * rejects unrelated .node files and supports future platform and architecture
 * spellings without a fixed target list.
 */
export function findNativeBinaries(
  ...
  const macosBinaries = entries
    .filter(
      (entry) =>
        entry.isFile() &&
        entry.name.startsWith(`${moduleName}.darwin-`) &&
        entry.name.endsWith('.node'),
    )
    .map((entry) => path.join(directory, entry.name))
  const sidecarBinaries = entries
    .filter(
      (entry) =>
        entry.isDirectory() &&
        entry.name !== 'darwin' &&
        !entry.name.startsWith('darwin-') &&
        /^[a-z0-9]+-[a-z0-9-]+$/i.test(entry.name),
    )
    .map((entry) => path.join(directory, entry.name, `${moduleName}.${entry.name}.node`))
    .filter((binaryPath) => existsSync(binaryPath))

  const binaries = [...macosBinaries, ...sidecarBinaries].sort((left, right) =>
